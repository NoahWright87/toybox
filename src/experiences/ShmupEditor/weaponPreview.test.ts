...
import { createBlankWeapon } from "./unitTypes";
...
describe("burstPeriodMs", () => {
  it("uses fireIntervalMs when positive", () => {
    expect(burstPeriodMs({ fireIntervalMs: 800 })).toBe(800);
  });
  it("falls back to the preview loop period for a single-burst weapon", () => {
    expect(burstPeriodMs({ fireIntervalMs: 0 })).toBe(PREVIEW_LOOP_FALLBACK_MS);
  });
});
...
describe("sweepOffsetDeg", () => {
  it("is always 0 for a static (non-sweeping) weapon", () => {
    expect(sweepOffsetDeg({ sweepSpeedDeg: 0, pingPong: false }, 5000)).toBe(0);
  });

  it("grows linearly with time when not ping-ponging", () => {
    const w = { sweepSpeedDeg: 90, pingPong: false };
    expect(sweepOffsetDeg(w, 1000)).toBeCloseTo(90);
    expect(sweepOffsetDeg(w, 2000)).toBeCloseTo(180);
  });

  it("ping-pong stays within +/- the fixed preview amplitude", () => {
    const w = { sweepSpeedDeg: 90, pingPong: true };
    for (let t = 0; t < 8000; t += 137) {
      const v = sweepOffsetDeg(w, t);
  ...
  it("ping-pong returns to 0 at the start of every period", () => {
    ...
    expect(sweepOffsetDeg(w, 0)).toBeCloseTo(-SWEEP_PINGPONG_AMPLITUDE_DEG, 1);
    expect(sweepOffsetDeg(w, 4000)).toBeCloseTo(-SWEEP_PINGPONG_AMPLITUDE_DEG, 1);
  });
});

describe("isTelegraphing", () => {
  it("is false when the weapon has no telegraph window", () => {
    expect(isTelegraphing({ fireIntervalMs: 1000, telegraphMs: 0 }, 999)).toBe(false);
  });

  it("is true only in the window immediately before a burst fires", () => {
    const w = { fireIntervalMs: 1000, telegraphMs: 200 };
    expect(isTelegraphing(w, 700)).toBe(false);
    expect(isTelegraphing(w, 850)).toBe(true);
    expect(isTelegraphing(w, 999)).toBe(true);
  });
});

describe("computePreviewBullets", () => {
  // fireIntervalMs is chosen well above PREVIEW_BULLET_LIFE_MS (1400ms) in
  ...
  // is correct, expected behavior for a rapid-fire weapon, not a bug (see
  ...
  it("spawns a bullet at the origin at the instant of a burst", () => {
    ...
    const bullets = computePreviewBullets(w, 0, 0);
  ...
  it("a bullet travels outward along its aim angle over time", () => {
    const w = weapon({ count: 1, arcStartDeg: 0, arcEndDeg: 0, fireIntervalMs: 5000, perShotDelayMs: 0 });
    const bullets = computePreviewBullets(w, 0 /* aim = +x */, 500);
    expect(bullets).toHaveLength(1);
  ...
  it("no bullets remain once every shot has exceeded its lifespan and no new burst has fired", () => {
    const w = weapon({ count: 1, arcStartDeg: 0, arcEndDeg: 0, fireIntervalMs: 100000, perShotDelayMs: 0 });
    const bullets = computePreviewBullets(w, 0, 5000);
    expect(bullets).toHaveLength(0);
  });

  it("multiple shots in a burst all spawn together when perShotDelayMs is 0", () => {
    const w = weapon({ count: 3, arcStartDeg: -30, arcEndDeg: 30, fireIntervalMs: 5000, perShotDelayMs: 0, spacing: "even" });
    const bullets = computePreviewBullets(w, 0, 0);
    expect(bullets).toHaveLength(3);
  });

  it("a rapid-fire weapon (interval shorter than bullet lifespan) shows overlapping bullets from consecutive bursts", () => {
    const w = weapon({ count: 1, arcStartDeg: 0, arcEndDeg: 0, fireIntervalMs: 1000, perShotDelayMs: 0 });
    const bullets = computePreviewBullets(w, 0, 0);
