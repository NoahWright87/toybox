...
describe("moveStep / updateStep", () => {
  it("moveStep updates only the target step's position", () => {
    let e = addStep(instance(), { x: 0, y: 0 });
    e = addStep(e);
  ...
  it("updateStep patches arbitrary fields on the target step only", () => {
    let e = addStep(instance());
    e = addStep(e);
    const updated = updateStep(e, e.steps[1].id, { visible: false, speedMultiplier: 0.5 });
    expect(updated.steps[1].visible).toBe(false);
    expect(updated.steps[1].speedMultiplier).toBe(0.5);
    expect(updated.steps[0].visible).toBe(true);
  });
...
describe("activeStepAt", () => {
  it("returns null before the instance's first step", () => {
    const e = addStep(instance()); // time 0
