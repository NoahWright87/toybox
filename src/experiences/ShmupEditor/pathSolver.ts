 ...
 * That inversion is the point (Noah): "make the editor just work with
 * where the points are placed... if I place a point for a jet at 0,0 and
 * a second at 300,0 and a third at 300,300, the jet can't make a sudden
 * 90 degree turn, so the editor will have to bend the movement to make it
 * work." The editor never rejects a placement and never marks anything
 * invalid — it solves for the closest flyable path and shows you that.
 *
 ...
 *
 * Solves are memoized (`solvePathCached`) because the editor re-solves on
 * every render and the game re-solves per instance.
 */
...
/** Structurally satisfied by `EncounterStep` (and the game's `AuthoredStep`) — the solver only needs position plus the optional hand-dragged handles. */
