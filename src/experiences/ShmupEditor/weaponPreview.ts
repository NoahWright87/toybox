/**
 * Pure simulation for `WeaponForm.tsx`'s live bullet-pattern preview
 * (Noah's request — "a small preview showing what the weapon would do
 * would be absolutely killer... shmups are very visual games"). Computes
 * bullet dot positions as a function of elapsed time, the same
 * "declarative, recomputed from scratch every frame" shape as
 * `movementPreview.ts`'s `computeInstancePreview` rather than
 * accumulating mutable simulation state — recomputing is cheap at this
 * scale and avoids any state to reset when a form field changes mid-preview.
 *
 ...
 * shared code with the game" stance the rest of the editor takes):
 * bullet travel speed is a fixed preview constant, not derived from
 * whatever Unit `spawnUnitId` points at (that Unit might not even be
 * resolvable while just browsing the picker), and a single-burst weapon
 * (`fireIntervalMs === 0`) still loops every `PREVIEW_LOOP_FALLBACK_MS`
 * so the preview keeps demonstrating the pattern instead of firing once
 * and going static.
 */
import type { WeaponDef, WeaponSpacing } from "./unitTypes";
...
/** WeaponDef has no dedicated sweep-amplitude field; ping-pong needs *some* fixed swing to oscillate within, so this is a documented preview-only choice, not derived from authored data. */
...
/** How often a burst repeats — real for `fireIntervalMs > 0`, a fallback loop period otherwise so the preview never just fires once and stops. */
export function burstPeriodMs(weapon: Pick<WeaponDef, "fireIntervalMs">): number {
  return weapon.fireIntervalMs > 0 ? weapon.fireIntervalMs : PREVIEW_LOOP_FALLBACK_MS;
}
...
export function sweepOffsetDeg(weapon: Pick<WeaponDef, "sweepSpeedDeg" | "pingPong">, elapsedMs: number): number {
  if (weapon.sweepSpeedDeg === 0) return 0;
  const elapsedSec = elapsedMs / 1000;
  if (!weapon.pingPong) return weapon.sweepSpeedDeg * elapsedSec;
  const speed = Math.abs(weapon.sweepSpeedDeg);
...
export function shotAngleOffsets(weapon: Pick<WeaponDef, "count" | "arcStartDeg" | "arcEndDeg" | "spacing">): number[] {
  const { count, arcStartDeg, arcEndDeg, spacing } = weapon;
  if (count <= 1) return [(arcStartDeg + arcEndDeg) / 2];
  if (spacing === ("random" as WeaponSpacing)) {
...
export function isTelegraphing(weapon: Pick<WeaponDef, "fireIntervalMs" | "telegraphMs">, elapsedMs: number): boolean {
  if (weapon.telegraphMs <= 0) return false;
  const period = burstPeriodMs(weapon);
  const phase = ((elapsedMs % period) + period) % period;
  return phase >= period - weapon.telegraphMs;
}

/** Every bullet dot's current render position at `elapsedMs`, given the weapon's config and an absolute aim angle (degrees, 0 = +x/right, 90 = +y/down, matching the aim-handle convention EncounterEditor.tsx already uses). Returns `[]` while nothing is mid-flight. */
export function computePreviewBullets(weapon: WeaponDef, aimDeg: number, elapsedMs: number): PreviewBullet[] {
  const period = burstPeriodMs(weapon);
