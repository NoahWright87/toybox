...
> **Status:** Framing pass locked (2026-06-22). Structure and formulas are settled; bare numeric constants live in `tuning.spec.md` and are the balance pass's job.
>
...
5. **Tuning is an asset.** All numeric levers live in one typed tuning module (`tuning.spec.md`) — the balance pass and Noah adjust there, never in systems code.
...
A **career** = ~5 **Seasons**, each a node map ending in a **Season Finale boss**; the last ends in the **Series Finale** → optional endless **Syndication**. One ship across the whole career. **One death ends the episode** (no hull lives), costing Ratings; **Ratings < 0 = Cancelled = career over.** See `run-structure.spec.md`.

## Spec map

| Spec | Covers | Issues |
|---|---|---|
| `stats.spec.md` | composition grammar, archetypes, 16 main stats | F3 #131 |
| `combat.spec.md` | damage, defense pipeline, mobility | F3 #131, F6 #134 |
| `weapons.spec.md` | effect engine, attack modifiers, gold upgrades | F4 #132, C1 #140, C2 #141 |
| `items-and-brands.spec.md` | items, stacking, brand tags, offer weighting | F4 #132, F9 #137, C3 #142, C4 #143 |
...
| `economy.spec.md` | currencies, coins/tips, interest, shop, gold sinks | F9 #137 |
...
| `tuning.spec.md` | the single home of every numeric lever | (all) |
