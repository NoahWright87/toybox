...
- Lives in a dedicated dir (e.g. `games/shmup/content/`): game title / "Noahsoft presents" card, weapon/item/chassis/enemy names + descriptions, **Ratings tier names** (Nobody → … → Kevin Bacon), **sponsor/brand** names + taglines + personalities, event-node text, Season/Finale/Syndication flavor, and the **crowd-comment pool** (`audience-and-score.spec.md`).
